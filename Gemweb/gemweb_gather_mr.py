...
class Gemweb_gather(MRJob):
    ...
    def reducer(self, launch, _):
        ...
        mongo['gemweb_debug_log'].update_one(query_doc, {"$push": {"logs": "reduce"}}, upsert=True)
        ...
        while date_from < date_to:
            mongo['gemweb_debug_log'].update_one(query_doc, {"$push": {"logs": f"getting from {date_from} to {date_to}"}})
        ...
        if len(data) > 0:
            ...
            mongo['gemweb_debug_log'].update_one(query_doc, {"$push": {"logs": f"saving to hbase"}})
        ...
        mongo['gemweb_debug_log'].update_one(query_doc, {"$push": {"logs": f"finish"}})
